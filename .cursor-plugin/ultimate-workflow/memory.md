...
- **research/new-sdlc-with-vibe-coding.md** *(deliberately not a link, and not shipped: it lives at the git repo root, not inside the installed plugin. A caveat that contradicts its own hyperlink is worse than plain prose.)* — Summary of Google's May 2026 paper on agentic engineering: the vibe-coding spectrum, `Agent = Model + Harness`, static vs dynamic context, and the CapEx/OpEx economics. **Read when** designing agent orchestration, deciding what belongs in always-on context, arguing whether a problem is the model or the harness, or justifying upfront structure. Full PDF alongside it.
