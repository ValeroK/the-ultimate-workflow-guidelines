...
- **Hook config is read at host startup.** A gate registered mid-session does not take effect until you restart Claude Code or Cursor, and until then it silently enforces nothing -- no block, no log, no error. Run `node hooks/status.js` to tell the three cases apart: never run, live, or disabled. (Discovered 2026-08-25)
