...
- **Hook config and agent definitions are both read at host startup.** A gate or an `agents/*.md` type added mid-session does not exist until you restart Claude Code. The two fail differently: a missing agent type fails loudly in seconds (`agent type '<name>' not found`, with the available types listed); a missing hook enforces nothing, with no block, no log, and no error. Run `node hooks/status.js` to tell the three hook cases apart: never run, live, or disabled. (Discovered 2026-08-25, hit again 2026-08-26)
