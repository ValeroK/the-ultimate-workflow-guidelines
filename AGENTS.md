...
This file is the always-on root doc for every host. `CLAUDE.md` is a one-line stub that imports it; Cursor reads this file natively. It is an **index, deliberately thin** — always-loaded text is paid for on every turn whether it is relevant or not, so only what has **no retrieval cue** belongs here. See `memory/context-boundary.md` before adding to it.
...
Full text, with the reasoning behind each: `skills/the-ultimate-workflow-guidelines/SKILL.md`.
