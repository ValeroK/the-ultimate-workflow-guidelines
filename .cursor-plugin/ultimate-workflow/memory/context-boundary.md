...
- **The memory protocol moved to `skills/the-ultimate-workflow-guidelines/references/memory-protocol.md`.** Its real consumer is already `workflows/harvest.js`, which inlines the decision test into the router agent's prompt — delivered at the moment of use, in a fresh context, by a fixed script rather than a model remembering to consult it. Keeping 715 always-on words was paying twice for the second-best copy.
